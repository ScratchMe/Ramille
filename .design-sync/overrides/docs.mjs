// forked from design-sync lib/docs.mjs — le kit nomme ses docs par composant `<Nom>.prompt.md`, pas `<Nom>.md`.
...
function findComponentDoc(c, { docsDirFiles, mapped, cfgPath }) {
  ...
  if (dir) {
    ...
    // Fork Ramille : le kit range la doc d'usage de chaque composant en
    // `<Nom>.prompt.md`, à côté de son `.jsx` et de son `.d.ts`. C'est déjà le nom
    // de sortie du convertisseur, et le renommer en `<Nom>.md` rendrait fausse une
    // ligne du HANDOFF figé de v1-14 ; on ajoute donc le motif à la sonde plutôt
    // que d'énumérer 28 chemins dans `docsMap`, qui se périmeraient au composant
    // suivant. Sonde en premier : c'est la doc la plus spécifique du kit.
    for (const f of [`${c.name}.prompt.md`, `${c.name}.md`, `${c.name}.mdx`, `${c.name}.docs.mdx`]) {
      const p = join(dir, f);
