...
| C4.1 | [#143](https://github.com/ScratchMe/TraceVerte/issues/143) | | | page de décision [`v1-23`](v1-23-point-quantitatif.md), 19/09/2026 — **recommande de fermer par écrit pour la V1**, avec sa condition de réouverture : une empreinte vivante entre deux bilans serait une seconde source de vérité pour le chiffre, ce que le dépôt refuse partout ailleurs, et la boucle n'a jamais tourné sur personne |
| C4.2 | [#144](https://github.com/ScratchMe/TraceVerte/issues/144) | | | page de décision [`v1-25`](v1-25-coup-de-pouce-la-veille.md), 19/09/2026 — cinq arbitrages, dont deux touchent le schéma. Le mot de la veille est piloté par l'**engagement** et non par le point (depuis C2.3 le point interroge la période écoulée), il triple le volume de messages, et une table à part passerait **à côté du plafond de `regime_de_rappel`** — en silence |
| C4.3 | [#145](https://github.com/ScratchMe/TraceVerte/issues/145) | | | page de décision [`v1-24`](v1-24-deplacements-professionnels.md), 19/09/2026 — **la moitié décidée le 10/09 (une phrase d'aide, explicitement sans arbitrage) n'a jamais été livrée** : elle vivait dans une case du tableau des arbitrages, et une case n'est pas une tâche. **Livrée le 19/09/2026** par la contre-lecture, qui a constaté son absence en écrivant la page. Reste : sortir le poste du lot 4 — il rouvre le modèle à trois postes, la restitution, les deux boucles et le référentiel d'actions |
| C4.4 | [#146](https://github.com/ScratchMe/TraceVerte/issues/146) | [#257](https://github.com/ScratchMe/Ramille/pull/257) | 21/09/2026 | **livré**, page de décision [`v1-21`](v1-21-modes-manquants.md) — **à trois volets et non quatre** (l'occupation longue distance était livrée depuis C3.5), quatre arbitrages allés dans le sens recommandé. Une seule migration, comme D4 l'impose. **Cinq modes et non quatre** : `train_ter` s'ajoute sur une règle écrite en tête de la migration — le générique est le repli des bilans d'avant la question, et une réponse reçoit un mode à elle quand elle change le facteur **ou les mots** ; par la même règle `velo_mecanique` n'existe pas. Mesuré après coup : le bilan d'un usager du RER passe de **249,2 à 88,0 kg/an** (rapport 2,8313), les deux longs trajets en autocar d'un profil valent **52,6 kg** là où ils valaient zéro. Deux choses non prévues par la page : `min_distance_km` sur les gabarits, sans quoi le gabarit VAE aurait **doublé** le gabarit vélo au lieu de prendre la fenêtre au-dessus ; et quatre gabarits d'action au lieu d'« un possible ». **Les deux entorses au point de résolution unique (`v1-27` §1) sont fermées du même geste**, comme cette dette le prescrivait. **Deux mutations ont dit l'inverse de ce que j'attendais** : remplacer le facteur du RER par celui du TER ne cassait **aucune** assertion — les produits dérivent du référentiel des deux côtés, donc ils épinglent la résolution et jamais la valeur, d'où l'assertion ajoutée sur l'ordre `TGV < Intercités < RER < TER` —, et ramener l'autocar sous le TER en casse **cinq**, dont deux du plan. **Et un sixième défaut a été trouvé dans la PR, en jouant les deux crons de 6 h plutôt qu'en relisant le diff** : `generate_extras_checkins` énumère les compteurs de voyages un par un pour décider si la boucle mensuelle existe, et l'autocar n'y était pas — un profil dont les seuls longs trajets sont en car avait un poste réel (105 kg/an pour quatre trajets), un plan portant « Remplacer un de tes longs trajets en autocar par le train », et **aucun point mensuel**, donc jamais la question que cette action existe pour refermer. Même forme que le défaut de la soumission trouvé une heure plus tôt : une liste de réponses écrite à la main. **Et un septième constat, en comparant le distant à ce que le dépôt en dit** : les neuf corps de fonction, `action_templates` et `transport_modes` s'apparient au md5 près entre la stack locale et le distant, mais pas l'horodatage — `v1-27` §9 demandait d'appliquer « sous le nom et l'horodatage du fichier », ce que `apply_migration` ne permet pas. La parade se joue après coup (relever, renommer), et ce fichier est **le premier du dépôt à s'apparier exactement au sien** ; règle portable en `SUPABASE.md` §1.5. Reste ouvert et consigné en `v1-27` §12.16 : le **gain** de l'action « en train » reste au tarif du TER, faute de pouvoir cibler le RER sans savoir où la personne habite au sens du réseau |
...
| C4.7 | [#149](https://github.com/ScratchMe/TraceVerte/issues/149) | | | page de décision [`v1-22`](v1-22-retirer-un-bilan.md), 19/09/2026 — `status = 'withdrawn'` rend **automatiquement justes** les douze lectures de `completed` (la fiche en annonçait trois), et c'est ce qui écarte une colonne `withdrawn_at`. La seule qui ne le devient pas est la lecture **par identifiant** de la restitution, qui est l'adresse qui circule |
| C4.8 | [#150](https://github.com/ScratchMe/TraceVerte/issues/150) | | | page de décision [`v1-26`](v1-26-meme-saison-un-an-apres.md), 19/09/2026 — **moitié livrée par C6.3** (la proposition de re-bilan à l'anniversaire saisonnier). Reste l'appariement, qui recouvre le moment anniversaire (C6.5) : une dérivation pour deux surfaces, ou deux chiffres qui se contrediront. Biais des facteurs mesuré nul aujourd'hui — aucun mode n'a de seconde version — et certain d'ici un an |
...
| 11.17 | C5.6 · C5.7 (le premier parcours) | **Ouverte le 17/09/2026, le jour de la livraison de la vague 11.** Le parcours entier — soumission du premier questionnaire, restitution **sans barre d'onglets**, carte « Ton premier plan », « Compris », la barre qui arrive et la carte « Plan et Suivi » qui la nomme — n'a jamais été vu par personne. **Sa plus grande part se vérifie au navigateur** (11.W.1), parce que rien n'y est natif : les deux marques vivent en stockage local et les trois cartes sont du rendu. Ce qu'un téléphone dira de plus, et qu'un navigateur ne peut pas dire : que l'absence de barre ne laisse **pas de bande vide** au-dessus de l'encoche d'un vrai Android (mesuré sur l'export, `[844, 0]` contre `[784, 60]`, mais l'encoche y vaut zéro) ; que l'entrée Reanimated de la carte parte bien sur un build natif — elle démarre à `opacity: 0`, et c'est la famille de défaut du 08/09/2026 ; et que revenir de l'arrière-plan pendant le parcours ne ramène pas la barre avant l'heure, `useRafraichirAuRetour` rechargeant l'écran à ce moment-là. |
| 11.18 | C5.2 (l'écran « Toutes les pistes ») | **Ouverte le 17/09/2026.** Demande un plan à **trois actions au moins**, sans quoi la porte ne s'affiche pas. Le rendu se regarde au navigateur (11.W.2) ; ce qu'il reste au téléphone : que la cible d'une ligne simple atteigne vraiment 44 px sous le doigt (elle les tient par son `paddingVertical`, pas par du `hitSlop`, les rangées se touchant) ; que deux lignes dépliées côte à côte respirent — c'est le défaut 13.5, corrigé deux fois, et la contre-lecture du 17/09 a trouvé qu'il revenait sous la carte **engagée** ; et que le retour après « C'est noté » ouvre bien la feuille des rappels, qui ne s'ouvre **pas** au navigateur sans adresse confirmée (`doitProposerLaFeuille` : sur web, elle exige `emailPossible`). |
| 11.19 | C4.4 (la densité du questionnaire) | **Ouverte le 21/09/2026, à la livraison de C4.4**, et c'est une question de **produit** posée à la recette plutôt qu'un défaut : le chantier ajoute deux questions imbriquées — le type de train sous « Train », le type de vélo sous « Vélo » — à celles qui existaient déjà (la motorisation sous « Voiture », le type de deux-roues, la taille du covoiturage, la part du second mode). **Sur les neuf entrées de B1.4, cinq ouvrent désormais une sous-question** — « Voiture (seul) », « Voiture (covoiturage) », qui en ouvre deux, « Deux-roues motorisé », « Train » et « Vélo » —, donc plus d'une sélection sur deux allonge l'écran. (Compter les `modeId` en donnerait quatre : les deux entrées « Voiture » partagent le leur.) Ce qu'on cherche : **remplir l'écran du mode au doigt, sur un vrai téléphone**, et répondre à une seule question — est-ce que c'est devenu pénible ? Trois choses à regarder pendant qu'on le fait, parce qu'elles ne se voient qu'à cette échelle : si l'encart imbriqué reste dans le champ de vision après la sélection (c'est la raison d'être de `PrecisionMode`, qui se rend **sous l'élément choisi** et non après la liste — posée après la liste entière, elle tombait à 774 px pour un conteneur de 684, soit 242 px hors champ, sous le pied collant) ; si l'on comprend qu'il reste quelque chose à faire quand « Suivant » est grisé ; et si changer d'avis sur le mode, une fois la sous-question remplie, se fait sans avoir l'impression de tout recommencer. **Aucune suite ne peut répondre à ça** — `manqueDeLEtape` garde que la réponse est exigée, le parcours réel garde qu'elle s'enregistre, et ni l'un ni l'autre ne dit si c'est agréable. Si la réponse est « oui, c'est pénible », la suite n'est pas un patch : c'est un brief de design, destination légitime (cf. `RECETTE.md` §2.4, deux des sept constats du 16/09/2026 y sont partis). |
...
### 11.W — Ce que le web peut vérifier, et qui n'attend pas octobre

**Aucun build EAS n'est disponible avant le 1er octobre** (quota du plan gratuit, registre
d'exploitation §3.3), et le tableau ci-dessus ne fait que grossir. Mais tout ce qu'il contient ne
demande pas un appareil : une bonne part du lot 5 est du **rendu et de l'état local**, donc
vérifiable dans un navigateur, sur `www.ramille.fr`, par la personne qui pilote — sans rien
installer et sans attendre.

**Deux précautions qui décident du résultat.** D'abord, **une fenêtre privée neuve à chaque
parcours** : les marques locales (`traceverte.*`) et le brouillon y vivent, donc un parcours rejoué
dans la même fenêtre ne repart pas de zéro et montrera l'état d'après. Ensuite, **c'est la
...
| 11.W.2 | **L'écran « Toutes les pistes »** (C5.2) | Depuis le plan, le lien « Voir toutes les pistes · N ». Attendu : les pistes **groupées par poste**, la tête de l'écran étant la même action que la première carte du plan ; chaque ligne s'ouvre en carte au toucher, **plusieurs à la fois** ; « C'est noté » ramène au plan avec l'action en tête. Demande un plan à trois actions au moins, sinon le lien ne s'affiche pas. |
| 11.W.3 | **L'encart de contexte et sa porte** (C5.5, **destination changée par C6.4**) | Bas du plan : une phrase qui énumère les réponses du contexte, **sans aucun chiffre et sans nommer d'action écartée**, et « Modifier ces réponses » qui ouvre désormais `/contexte` — **pas** le questionnaire. Ce qui serait un défaut : retomber dans le questionnaire, donc sur un parcours qui finit par une soumission. |
| 11.W.4 | **Le télétravail en jours** (C5.4) | Dernière étape du questionnaire : « Sur tes N jours de trajet, combien pourrais-tu travailler depuis chez toi ? », trois puces **Aucun / Un jour / Deux ou plus**, et « Parfois » nulle part. La question **disparaît** si on redescend à un seul jour de trajet — et « Suivant » ne doit alors pas rester bloqué. |
| 11.W.5 | **Le classement du plan** (C5.1) | Sur un profil avec un gros trajet domicile-travail : la **première** carte doit être la meilleure piste du poste dominant, et la **deuxième** peut venir d'un autre poste si son gain est supérieur. C'est l'inverse d'avant, où toute action du poste dominant passait devant, même à 48 kg contre 461. |
| 11.W.6 | **L'espace fine des milliers** (C5.8) | Demande un gain ou un cap à quatre chiffres, donc un profil chargé (trajet long en voiture thermique, cinq jours par semaine) : « − 1 601 kg » et jamais « − 1601 kg ». |
| 11.W.7 | **La restitution et le suivi d'un re-bilan** (C2.7, C3.1) | Refaire un bilan depuis « Revoir mon bilan » en changeant une réponse : l'écart par poste, les six barres à échelle commune, et la note qui dit l'écart **en kilos** avant le pourcentage. |
| 11.W.8 | **Le contexte corrigé sans bilan** (C6.4) — la ligne que le chantier demande | Deux portes à essayer : l'encart du plan, et « Mon contexte de mobilité » dans « Toi ». Sur l'écran : les quatre mêmes questions que la dernière étape du questionnaire, **« Enregistrer » inactif tant que rien n'a bougé**, et un pied qui dit « ton bilan n'est pas refait ». Puis changer l'accès aux transports en commun pour **Inexistant** et enregistrer. Attendu : retour d'où l'on venait, **le plan s'est réduit** (les actions qui supposaient un métro ont disparu), et surtout **le suivi ne gagne aucune entrée** — un second bilan dans l'historique serait le défaut que ce chantier ferme. Deux vérifications qui valent le détour : (a) sur un profil « sorties rares », la phrase d'intro doit nommer l'exception du nombre de véhicules, et changer cette réponse doit faire **bouger le chiffre du bilan** sans nouvelle entrée de suivi ; (b) si l'action engagée disparaît du plan recalculé, l'encart qui l'annonce doit dire « avec tes nouvelles réponses de contexte » et **jamais** « avec ton nouveau bilan ». |

| 11.W.9 | **Le contenu des deux e-mails du compte** (réécrits le 20/09/2026 : plus aucun lien, un code à huit chiffres) — demandé par la personne qui pilote | **Les seuls contrôles qui se lisent dans une boîte de réception, et pas à l'écran.** Deux passages, deux adresses distinctes — le distant n'accepte qu'un envoi par minute et par adresse. **(a) Le rattachement.** Fenêtre privée → `www.ramille.fr` → faire un bilan, ou depuis « Toi » → « Rattacher un compte » → « Utiliser un email à la place » → l'écran « Rattacher mon adresse » → saisir l'adresse → « Recevoir un code ». **L'adresse doit être une adresse qu'on contrôle et qui n'a AUCUN compte Ramille**, et depuis le 21/09/2026 c'est pour une autre raison qu'avant : sur une adresse déjà rattachée, `updateUser` répond `email_exists`, et l'app envoie alors un code de **reconnexion** en montrant le **même** écran de code (`v1-28` §7.1, la fermeture de l'oracle). Un e-mail part donc bien — mais c'est l'**autre**, celui du (b) ci-dessous, et rien à l'écran ne le dit. La séance se perdrait à relire le gabarit de rattachement dans un message qui n'en est pas un. Cette ligne a dit l'inverse jusqu'à cette date (« aucun e-mail ne part ») : c'était vrai avant l'arbitrage. **Il n'y a plus de lien à ne pas cliquer** : taper le code crée un vrai compte en production, donc on ne le tape que si on l'accepte, et lire l'e-mail ne risque plus rien. Cinq choses à regarder : l'objet est **« Cette adresse vient d'être saisie dans Ramille »** ; le corps ouvre par « L'adresse … vient d'être saisie dans Ramille » et **l'adresse y est bien écrite** (un blanc à cet endroit est le seul défaut que rien d'autre ne verrait — la substitution a été éprouvée localement, jamais depuis la production) ; **le code fait huit chiffres**, lisibles d'un coup d'œil, et **aucun lien n'apparaît nulle part** ; la sortie dit **« Si ce n'est pas toi, ne fais rien »** ; et le tout est en français, signé « — Ramille », pas par Supabase. **(b) La reconnexion.** Sur une adresse qui a un compte, depuis un navigateur neuf : `www.ramille.fr/connexion/retrouver` → l'adresse → « Recevoir un code ». Objet **« Ton code pour retrouver ton compte »**, un code à huit chiffres, aucun lien, et la phrase « Si tu n'as rien demandé, tu peux ignorer ce message : sans ce code, personne n'entre. » **Puis la seule question qui demande un humain, et c'est pour elle que le texte a été réécrit** : relis le premier en te mettant à la place de quelqu'un qui n'a rien demandé. Est-ce clair en deux secondes que ça ne parle pas d'un compte à toi, et qu'il n'y a rien à faire ? Si la réponse est non, le texte a raté son but, quand bien même chaque mot serait au bon endroit. **Et une seconde question, neuve** : les huit chiffres se recopient-ils sans effort depuis ta messagerie vers l'app — coupés en deux par un retour à la ligne, collés avec une espace, illisibles en petit ? C'est le geste que le passage au code ajoute, et le seul endroit où son coût se voit. |

**Ce que le navigateur ne peut pas dire, et qu'il ne faut pas croire vérifié pour autant** : les
notifications push et la cérémonie qui les propose (`doitProposerLaFeuille` exige `emailPossible`
sur web, donc la feuille ne s'ouvre pas sur une session anonyme) ; le retour de l'app depuis
l'arrière-plan, qui est le chemin nominal de la boucle d'engagement ; TalkBack ; les marges de
sécurité d'un vrai écran ; les animations Reanimated telles qu'Android les rend ; et les liens
`ramille://`. Ces lignes-là restent dans le tableau du dessus.
