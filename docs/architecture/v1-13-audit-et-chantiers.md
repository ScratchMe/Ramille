...
| C4.4 | [#146](https://github.com/ScratchMe/TraceVerte/issues/146) | | | page de décision [`v1-21`](v1-21-modes-manquants.md), 19/09/2026 — **à trois volets et non quatre** (l'occupation longue distance est livrée depuis C3.5). Mesuré sur l'endpoint ACV le même jour : le mode « Train ou RER » porte le slug `ter` alors qu'un slug `rer` existe à **2,83 fois moins**, et le gain du report modal est du coup **sous-estimé de 64 kg/an** sur un profil francilien ; le VAE est compté **64 fois trop bas** ; l'autocar émet **plus qu'un TER**, ce qui mérite un test |
