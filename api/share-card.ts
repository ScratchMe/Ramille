...
// tourne dans un runtime Web Fetch API (Request/Response), pas dans le contexte React Native
// de l'app.
//
// Runtime Node.js (pas de `export const config = { runtime: 'edge' }`) plutôt qu'Edge :
// le bundle Edge de @vercel/og (Satori + resvg.wasm + yoga.wasm + police par défaut, ~2,4 Mo
// non compressés) dépasse la limite de taille des Edge Functions du plan Hobby de ce projet —
// c'est la cause du premier échec de déploiement de cette carte, confirmée par bisection
