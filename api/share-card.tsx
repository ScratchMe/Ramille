...
function Mascot() {
  return (
    <svg width="140" height="140" viewBox="0 0 100 100">
      <path d="M50,8 C78,26 84,56 50,92 C16,56 22,26 50,8 Z" fill={ACCENT} />
      <path
        ...
        opacity={0.55}
      />
      <circle cx={33} cy={60} r={5} fill={BLUSH} opacity={0.55} />
      <circle cx={67} cy={60} r={5} fill={BLUSH} opacity={0.55} />
      <circle cx={39} cy={50} r={4.2} fill={INK} />
      <circle cx={61} cy={50} r={4.2} fill={INK} />
      <path d="M42,62 Q50,68 58,62" stroke={INK} strokeWidth={3.2} strokeLinecap="round" fill="none" />
    </svg>
  );
}
...
export default function handler(request: Request) {
  ...
  return new ImageResponse(
    (
      ...
      >
        <Mascot />
        <div style={{ display: 'flex', fontSize: 30, fontWeight: 600, color: ACCENT_TEXT, marginTop: 28 }}>
          Mon empreinte transport
        </div>
        <div style={{ display: 'flex', fontSize: 104, fontWeight: 700, color: INK, marginTop: 12 }}>
          {total} t CO₂e / an
        </div>
        {poste && (
          <div style={{ display: 'flex', fontSize: 34, color: TEXT_SECONDARY, marginTop: 20 }}>{poste}</div>
        )}
        <div style={{ display: 'flex', fontSize: 26, color: TEXT_SECONDARY, marginTop: 56 }}>
