...
-- 1 vol × 9000 km × 0,177894 = 1601,05 kg. Avec l'ancienne valeur (celle du court-courrier,
-- 0,224572), on afficherait 2021,15 kg — soit 26 % de trop sur le poste le plus lourd des
-- profils « voyages ».
...
select is(
  ...
  1601.05::numeric,
  'A : 1 vol long-courrier -> 9000 km au facteur long (0,177894), plus la valeur court-courrier'
);

-- ── Scénario B : vol court/moyen-courrier ───────────────────────────────────────────────
-- 1 vol × 1500 km × 0,184661 = 276,99 kg. Le produit modélise ces vols à 1500 km, ce qui
-- relève du segment MOYEN-courrier de l'ADEME (0,184661) et non du court-courrier (0,224572).
...
select is(
  ...
  276.99::numeric,
  'B : 1 vol court/moyen-courrier -> facteur du segment correspondant à la distance du poste (1500 km)'
);

-- ── Scénario C : train longue distance ──────────────────────────────────────────────────
-- 5 trajets × 800 km × 0,002930 = 11,72 kg. Au facteur TER, le même poste pèserait 110,76 kg
-- — presque dix fois trop, précisément là où le produit veut faire percevoir l'écart avec
-- l'avion.
...
select is(
  ...
  11.72::numeric,
  'C : 5 trajets longue distance en train -> facteur TGV (0,002930), plus le TER'
);

-- ── Scénario D : le train quotidien reste au TER (non-régression) ──────────────────────
-- 10 km × 2 × 5 j × 45 sem = 4500 km × 0,027690 = 124,61 kg.
...
--   jambe voiture : 4500 × 0,142253 = 640,1385, covoiturée à 3 -> 213,3795
--   jambe train   : 4500 × 0,027690 = 124,605,  jamais covoiturée
--   total = 337,98   (si le covoiturage divisait tout : (640,1385 + 124,605) / 3 = 254,91)
